...
function novaSessao(db: BancoLocal, u: UsuarioLocal): Sessao {
  ...
  const expiraEm = new Date(Date.now() + HORAS_SESSAO * 60 * 60 * 1000).toISOString();
...
function renovarSessao(db: BancoLocal, token: unknown): string {
  const s = db.sessoes.find((x) => x.token === String(token));
  if (!s) return "";
  const restante = new Date(s.expiraEm).getTime() - Date.now();
  const janela = HORAS_SESSAO * 60 * 60 * 1000;
  if (restante <= janela / 2) {
    s.expiraEm = new Date(Date.now() + janela).toISOString();
  }
  return s.expiraEm;
}
...
export async function requisicaoLocal<T>(payload: Record<string, unknown>): Promise<T> {
  ...
  switch (acao) {
    ...
    case "eu": {
      const eu = autenticar(db, payload.token);
      saida = { usuario: publico(eu), expiraEm: renovarSessao(db, payload.token) };
      break;
    }
