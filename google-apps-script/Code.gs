...
function processar(req) {
  try {
    ...
    switch (acao) {
      case 'eu':
        return ok({ usuario: publico(auth.usuario), expiraEm: auth.sessao.expiraEm });
...
function exigirSessao(token) {
  ...
  return { sessao: renovarSessaoSePreciso(achada), usuario: usuario };
}

/** Estende a validade quando passou da metade do prazo — a conexão não cai. */
function renovarSessaoSePreciso(sessao) {
  var restante = new Date(sessao.expiraEm).getTime() - Date.now();
  var janela = SEGURANCA.horasSessao * 60 * 60 * 1000;
  if (restante > janela / 2) return sessao;
  sessao.expiraEm = new Date(Date.now() + janela).toISOString();
  gravar('Sessoes', sessao);
  return sessao;
}
